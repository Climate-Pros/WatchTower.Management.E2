...
public static class DotEnv
{
    public static void Load(string filePath)
    {
        ...
        var rootDirectory = new FileInfo(filePath)?.Directory?.Parent?.Parent?.Parent?.Parent;
            
        if (rootDirectory is null) return;

        var envFile = rootDirectory.GetFiles(".env")?.FirstOrDefault();

        if (envFile is null) return;
        
        var envFilePath = envFile.FullName;
